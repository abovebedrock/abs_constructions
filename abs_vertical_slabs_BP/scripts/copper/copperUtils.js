...
export function setStage(block, stageNumber){
    if(isInCopperCategory(block.typeId) && !isWaxed(block.typeId)){
        const
            newID = `${getNamespace(block.typeId)}:${stageNumberToString[stageNumber]}${getCopperType(block.typeId)}`,
...
function getCopperType(typeId){
    return typeId.replace(getNamespace(typeId), "").replace(":", "").replace("waxed_", "").replace("exposed_", "").replace("weathered_", "").replace("oxidized_", "");
}
//#endregion
