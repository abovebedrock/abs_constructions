...
export default function waxOn(data){
    ...
    if(isModItem(id) && isInCopperCategory(id) && !isWaxed(id) && data.itemStack && data.itemStack.typeId === "minecraft:honeycomb") system.run(()=>{
        const
            ...
            hand = /**@type {EntityEquippableComponent}*/ (data.source.getComponent(EntityComponentTypes.Equippable)).getEquipmentSlot(EquipmentSlot.Mainhand),
            newPermutation = BlockPermutation.resolve(`abs:waxed_${id.substring(4, id.length)}`, data.block.permutation.getAllStates());
